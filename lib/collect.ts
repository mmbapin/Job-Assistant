...
export function matchesGreenhouseTitle(title:string,filter:Source['titleFilter']){return isFrontendTitle(title)||(filter==='frontend-fullstack'&&/\bfull[\s-]*stack\b/i.test(title));}
export async function collectFilteredGreenhouse(source:Source,read:typeof safeFetch=safeFetch):Promise<Job[]>{
 ...
 const matches=body.jobs.filter((r:Record<string,any>)=>typeof r.title==='string'&&matchesGreenhouseTitle(r.title,source.titleFilter));
 const jobs:Job[]=[];
