...
export async function detectSource(source:Source):Promise<Source>{
 ...
 if(career.hostname==='careers.remoteintegrity.com')return {...source,url:'https://careers.remoteintegrity.com/',provider:'Custom API',board:'remoteintegrity-frontend-fullstack',titleFilter:'frontend-fullstack',detection:'Remote Integrity · Frontend + Full Stack · Employer country restrictions apply'};
 if(['careersatagoda.com','www.careersatagoda.com'].includes(career.hostname)&&/^\/vacancies\/?$/.test(career.pathname))return {...source,url:'https://careersatagoda.com/vacancies/?keyword=Front+End&country=',provider:'Greenhouse',board:'agoda',titleFilter:'frontend-fullstack',category:'Global',country:'Global',detection:'Agoda / Greenhouse · Front End + Full Stack titles · All locations'};
